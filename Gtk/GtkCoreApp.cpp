...
void GtkCoreApp::on_startup() {
    ...
    //File|New sub menu:
    add_action("newstandard", sigc::mem_fun(*this, &GtkCoreApp::on_menu_file_new_generic));

    add_action("newfoo", sigc::mem_fun(*this, &GtkCoreApp::on_menu_file_new_generic));

    add_action("newgoo", sigc::mem_fun(*this, &GtkCoreApp::on_menu_file_new_generic));
    ...
    try {
        _builder->add_from_file("Gtk/menu.ui");
    }
...
void GtkCoreApp::create_window() {
    ...
    win->signal_hide().connect(sigc::bind<Gtk::Window*>(sigc::mem_fun(*this, &GtkCoreApp::on_window_hide), win));
