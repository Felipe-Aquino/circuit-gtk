...
public:
    ...
    void edit_component(){
        CompInfo& info = _app->editSelectedComponentEvent();
        Gtk::Window* toplevel = dynamic_cast<Gtk::Window*>(_draw->get_toplevel());
        EditDialog w(*toplevel, info);
        w.run();
    }
...
bool GtkDraw::on_button_press_event(GdkEventButton* button_event)  {
    ...
    if( (button_event->type == GDK_BUTTON_PRESS) && (button_event->button == 3) ) {
        if(_app.selectComponentEvent(button_event->x, button_event->y)){
            ...
            if(_popupMenu) {
                _popupMenu->popup(3, button_event->time);
                //    _popupMenu->popup_at_pointer((GdkEvent*)button_event);
                report::warn(report::format("In %:% -> \n         Gtk::Menu:popup is deprecated in v3.22.", __FILE__, __LINE__));
            }
        }
    }
    else {
...
void GtkDraw::setup_popup_menu(){
    ...
    try {
        builder->add_from_file("Gtk/popup.ui");
    }
    catch (const Glib::Error& ex) {
        std::cerr << "Building menus failed: " << ex.what();
    }

    auto gmenu = Glib::RefPtr<Gio::Menu>::cast_dynamic(builder->get_object("component-popup"));
